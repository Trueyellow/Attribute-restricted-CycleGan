...
class CycleGAN():
    ...
    def fit(self, img_A_generator, img_B_generator_list):
        ...
        if os.path.exists(os.path.join(opt.pic_dir, 'a2b_yh.h5')):
            self.AtoB.load_weights(os.path.join(opt.pic_dir, 'a2b_yh.h5'))

        if os.path.exists(os.path.join(opt.pic_dir, 'disA_yh.h5')):
            self.DisA.load_weights(os.path.join(opt.pic_dir, 'disA_yh.h5'))

        if os.path.exists(os.path.join(opt.pic_dir, 'b2a_yh.h5')):
            self.BtoA.load_weights(os.path.join(opt.pic_dir, 'b2a_yh.h5'))

        if os.path.exists(os.path.join(opt.pic_dir, 'disB_yh.h5')):
            self.DisB.load_weights(os.path.join(opt.pic_dir, 'disB_yh.h5'))
        ...
        while iteration < opt.niter:
            ...
            if iteration % opt.save_iter == 0 and iteration != 0:
                ...
                self.AtoB.save(os.path.join(opt.pic_dir, 'a2b_yh.h5'))
                self.BtoA.save(os.path.join(opt.pic_dir, 'b2a_yh.h5'))
                self.DisA.save(os.path.join(opt.pic_dir, 'disA_yh.h5'))
                self.DisB.save(os.path.join(opt.pic_dir, 'disB_yh.h5'))
            iteration += 1
