...
def build_model():
    ...
    pool2_au = MaxPool2D(pool_size=(2, 2))(conv2_au)

    conv3_au = Conv2D(128, (3, 3), activation='relu', padding='same')(pool2_au)
    pool3_au = MaxPool2D(pool_size=(2, 2))(conv3_au)

    conv4_au = Conv2D(256, (3, 3), activation='relu', padding='same')(pool3_au)
    pool4_au = MaxPool2D(pool_size=(2, 2))(conv4_au)
    #
    ...
    model = Model(input=inp_au, output=outputs, name="audio.h5")
