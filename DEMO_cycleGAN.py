...
# customized cycleGAN for our teammate with pre-trained cnn loss
import os
import keras.backend as K
from build_DG import basic_D, resnet_6blocks, basic_D_A, resnet_6blocks_A, face_loss_model, face_loss_model_sgm
...
class CycleGAN():
    def __init__(self, opt):
        ...
        face_loss_cal = face_loss_model_sgm()
        ...
        emotion_loss = K.mean(K.abs(face_loss_cal(real_A) - face_loss_cal(rec_A)))

        loss_DA = loss_DA_real_image_real_label + loss_DA_real_image_fake_label + \
                  loss_DA_fake_image_real_label + loss_DA_fake_image_fake_label + emotion_loss
        ...
        self.DisA = dis_A
        self.DisB = dis_B
        self.cnn_loss = face_loss_cal
        self.opt = opt

    def fit(self, img_A_generator, img_B_generator_list):
        opt = self.opt
        ...
        if os.path.exists(os.path.join(opt.pic_dir, 'a2b_cnn_yh.h5')):
            self.AtoB.load_weights(os.path.join(opt.pic_dir, 'a2b_cnn_yh.h5'))

        if os.path.exists(os.path.join(opt.pic_dir, 'disA_cnn_yh.h5')):
            self.DisA.load_weights(os.path.join(opt.pic_dir, 'disA_cnn_yh.h5'))

        if os.path.exists(os.path.join(opt.pic_dir, 'b2a_cnn_yh.h5')):
            self.BtoA.load_weights(os.path.join(opt.pic_dir, 'b2a_cnn_yh.h5'))

        if os.path.exists(os.path.join(opt.pic_dir, 'disB_cnn_yh.h5')):
            self.DisB.load_weights(os.path.join(opt.pic_dir, 'disB_cnn._yhh5'))

        self.cnn_loss.load_weights("yh_face_16_16_s.h5", by_name=True)
        self.cnn_loss.trainable = False

        while iteration < opt.niter:
            print('iteration: {}'.format(iteration))
            # samples
            real_A, label_A = img_A_generator(bs, return_label=True)
            img_B_generator = np.random.choice(img_B_generator_list)
            ...
            if iteration % opt.save_iter == 0 and iteration != 0:
                imga = real_A
                ...
                if np.sum(imga2b + imga2b2a + imgb2a + imgb2a2b)/(3 * 128 * 128 * 4) < 0.1:
                    raise RuntimeError("GENERATING VANISHING!!!")
                ...
                self.AtoB.save(os.path.join(opt.pic_dir, 'a2b_cnn_yh.h5'))
                self.BtoA.save(os.path.join(opt.pic_dir, 'b2a_cnn_yh.h5'))
                self.DisA.save(os.path.join(opt.pic_dir, 'disA_cnn_yh.h5'))
                self.DisB.save(os.path.join(opt.pic_dir, 'disB_cnn_yh.h5'))
            iteration += 1
