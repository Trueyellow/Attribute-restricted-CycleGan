...
import os
from base_layer import basic_D
from base_layer import resnet_6blocks
...
class CycleGAN():
    ...
    def __init__(self, opt):
        ...
        dis_B = defineD(opt.shapeB, opt.ndf, use_sigmoid=not opt.use_lsgan,
                        name='dis_B')

        gen_A = defineG(opt.shapeB, opt.shapeB[2], ngf=opt.ngf,
                        name='gen_A')
        ...
        if opt.idloss > 0:
            ...
            G_trainner.compile(Adam(lr=opt.lr_G, beta_1=opt.beta1, ),
                               loss=['MSE', 'MSE', 'MAE', 'MAE', 'MAE', 'MAE'],
                               loss_weights=[1, 1, opt.lmbd, opt.lmbd, opt.idloss*opt.lmbd, opt.idloss*opt.lmbd])
        else:
            G_trainner = Model([real_A, real_B],
        ...
        D_trainner.compile(Adam(lr=opt.lr_D, beta_1=opt.beta1, ), loss=['MSE', 'MSE', 'MSE', 'MSE'])
